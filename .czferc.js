...
module.exports = {
		...
		return [
			{
				type: 'input',
				name: 'ticket',
				message: 'Notion ticket Number',
				message: 'Notion ticket Number (just number) \n',
			},
			{
				type: 'list',
				name: 'type',
				message: 'Select type222',
				choices: types,
			},
			{
				type: 'input',
	...
	commitMessage({ answers }) {
		const ticket = answers.ticket ? `[DESK-${answers.ticket}]` : '[hotfix]';
		const scope = answers.scope ? `(${answers.scope})` : '';
		const head = `${ticket}${answers.type}${scope}: ${answers.subject}`;
